...
A multi-tenant marketplace where each creator gets their own storefront and direct Stripe Connect payouts. Customers buy digital products, access protected content, and can leave reviews. There's an optional AI review helper too - structured output only, Zod-validated, behind a feature flag.

## Screenshots

### Marketplace (filters & browse)
![Marketplace homepage with category filters](docs/screenshots/marketplace.png)

### Creator storefront
![Tenant storefront](docs/screenshots/storefront.png)

### Product detail
![Product page](docs/screenshots/product.png)

### Customer library
![Library with purchased content](docs/screenshots/library.png)
...
**Stripe Connect Express over rolling my own**

I used Connect Express, not a standard Stripe setup. Payout compliance and KYC are genuinely hard problems. Stripe has already solved them. Building that myself before the product has a single user would be the wrong order of operations.

**Payload CMS over a custom database layer**

Payload gave me an admin panel, media management, typed collections, and access control - ready. The alternative was writing all of that myself, badly, in three times the time. Payload v3 is stable enough that the coupling is worth it.

**Feature flags over separate deployments**

Subdomain routing and the AI review helper are toggled via environment variables, not separate deploys. I'm not shipping untested features against live traffic and managing two builds. One build, one toggle, same behaviour.

**Denormalised counters on Products**

Trending and bestseller sorting run on every marketplace request. A `COUNT(*)` join per page load is the kind of thing that looks fine in development and causes problems at any real volume. Payload hooks keep the counters current on every order and review change, so sorting is just reading an indexed field.

**Idempotent webhook handler**

Stripe retries failed webhook deliveries. Without deduplication, a timeout during order creation means the customer gets charged and ends up with a duplicate order. The handler checks `stripeCheckoutSessionId` uniqueness before inserting, so retries are harmless.

**Structured AI output over free text**

I forced `tool_choice: { type: 'tool' }` instead of parsing a free text response. Parsing a star rating out of a paragraph of prose is a reliability problem I didn't want to own. The output still goes through Zod validation before it touches the client.
...
Rate limiting on `generateReviewDraft` isn't implemented. In-memory state is NOOP in serverless - Upstash Redis would fix this in an afternoon, but this is a portfolio project.

Subdomain routing needs a DNS wildcard (`*.domain.com`) pointing to the same origin in production.

Media uploads fall back to the local filesystem in development when `BLOB_READ_WRITE_TOKEN` isn't set.
